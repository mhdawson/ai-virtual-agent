...
async def make_api_request(
    ...
    try:
        ...
        raise ValueError(
            f"API Error: {e.response.status_code} - {error_detail} when calling {e.request.method} {e.request.url}"
        ) from e
    except httpx.RequestError as e:
        raise ValueError(
            f"Request Error: Could not connect to Store Server at {e.request.url}. Details: {str(e)}"
        ) from e
