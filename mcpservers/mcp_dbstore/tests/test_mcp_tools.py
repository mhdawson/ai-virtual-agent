...
from .. import crud, database  # For direct DB manipulation/verification in tests
from ..models import ProductCreate  # Pydantic models for creating data
from ..models import ProductOrderRequest
...
async def initialized_test_mcp_server():
    """Fixture to provide an initialized FastMCP server instance with a clean database."""
...
async def test_tool_order_product_insufficient_inventory(initialized_test_mcp_server):
    ...
    async with Client(initialized_test_mcp_server) as client:
        ...
        # FastMCP typically returns ok=False if the tool function raises an unhandled exception.
        # The exception message might be in response.error or response.content (if FastMCP formats it).
        assert not order_response.ok
        # You might need to inspect order_response.error or order_response.content for the specific error message
        # Example: assert "Insufficient inventory" in str(order_response.error) or str(order_response.content)
...
async def test_tool_search_products(initialized_test_mcp_server):
    ...
    async with Client(initialized_test_mcp_server) as client:
        ...
        # For simplicity, we rely on products potentially added in other tests or ensure this test is robust to it.
