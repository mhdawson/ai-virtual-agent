...
from ..models import Order as PydanticOrder
...
async def db_session() -> AsyncSession:
    # Ensure tables are created for each test function run in an in-memory DB or test DB
    ...
    try:
        ...
        await session.close()
        # Optionally, drop tables again after tests if not using an in-memory DB that vanishes
        # async with database.engine.begin() as conn:
...
async def test_order_product_insufficient_inventory(db_session: AsyncSession):
    ...
    # The crud function itself raises before commit in this case, so session.commit() wouldn't be reached in the tool.
    # Here, we check product inventory remained unchanged.
    await db_session.rollback()  # Rollback any potential changes from the failed order attempt
